...
function getAdBlocks() {
    let tree = $.parseXML(getDataFromSessionStorage("radarRamiradarRamiradarRamiTree"));
    if (!tree) return;
    let blocks = [];
...
function getNumberOfParents(node) {
    let additionalTreePairs = JSON.parse(getDataFromSessionStorage("radarRamiradarRamiradarRamiAdditionalTreePairs"));
...
function getNumberOfChildren(node) {
    let additionalTreePairs = JSON.parse(getDataFromSessionStorage("radarRamiAdditionalTreePairs"));
    if (!additionalTreePairs) return;
